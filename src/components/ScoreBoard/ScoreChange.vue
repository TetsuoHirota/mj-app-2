    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <transition name="tobashi">
          <v-row v-if="tobi" class="form__li">
            <h5>飛ばした人</h5>
            <v-select
              v-model="tobashiId"
              :items="players"
        ...
        <v-subheader class="caption pa-0 mt-3"
          >※同点の場合、上に名前のある人が高い順位になります。</v-subheader
        >
        <transition name="error">
          <v-alert type="error" class="py-2 error" v-if="errorMessage">
            {{ errorMessage }}
          </v-alert>
        </transition>
      </v-card-text>
...
export default class ScoreChange extends Mixins(RuleConfig) {
  ...
  open(index: number) {
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      let pt = null;
      if (scores) {
        const personalScore = scores.find(
          (e: any) => e.uid === this.players[i].uid
        );
        if (personalScore) {
  ...
  validate() {
    if (this.playersInput !== this.rule.players) {
      ...
      this.errorMessage = `合計が${this.totalInputToBe}${
        this.isPtMode ? "pt" : "点"
      }になるように入力してください (現在合計: ${this.totalInput}${
        this.isPtMode ? "pt" : "点"
      })`;
    } else {
      this.errorMessage = "";
  ...
  getPt() {
    ...
    scores.sort((a: any, b: any) =>
      this.isPtMode ? b.pt - a.pt : b.score - a.score
    );
    ...
    for (let i = 1; i < scores.length; i++) {
      const uma =
        i === 1
          ? this.rule.uma.second
          : i === 2
          ? this.rule.uma.third
          : i === 3
          ? this.rule.uma.fourth
          : 0;
  ...
  saveScore() {
    if (this.validate()) {
      ...
      this.$store.dispatch("ScoreBoard/changeScores", {
        index: this.index,
        scores: scores,
      });
      this.close();
